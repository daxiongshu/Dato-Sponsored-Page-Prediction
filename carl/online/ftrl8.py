...
# A, paths
train='train_tr.csv'
test='train_va.csv'#'vali_100.tsv'
submission = 'ftrlcv8.csv'  # path of to be outputted submission file

# B, model
alpha = .01  # learning rate
beta = 2   # smoothing parameter for adaptive learning rate
...
class ftrl_proximal(object):
    ...
    def _indices(self, x):
        ...
        if self.interaction:
            ...
            x = sorted(x)
            for i in xrange(L):
                for j in xrange(i+bias, min(i+distance+bias,L)):
...
for e in xrange(epoch):
    ...
    #learner.alpha=learner.alpha*decay
